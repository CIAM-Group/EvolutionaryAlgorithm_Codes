...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,X)
            ...
            Y(:,J1) = 100 * Y(:,J1);
            PopObj(:,1) = 1 - cos(0.5*X(:,1)*pi).*cos(0.5*X(:,2)*pi) + 2*mean(Y(:,J1).^2,2);
            PopObj(:,2) = 1 - cos(0.5*X(:,1)*pi).*sin(0.5*X(:,2)*pi) + 2*mean(Y(:,J2).^2,2);
            PopObj(:,3) = 1 - sin(0.5*X(:,1)*pi)                     + 2*mean(Y(:,J3).^2,2);
        end
