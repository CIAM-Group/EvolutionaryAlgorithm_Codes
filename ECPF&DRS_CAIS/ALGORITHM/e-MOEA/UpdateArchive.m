function Archive = UpdateArchive(Archive,Offspring,epsilon,K)
% Update the archive
...
%--------------------------------------------------------------------------
    ...
    if ~any(all(ArchiveGrid<=repmat(ChildGrid,N,1),2))
       ...
       %% if length(Archive) > N , delete 
        while size(Archive.objs,1) > K
        Archive = setdiff(Archive,Archive(unidrnd(size(Archive.objs,1))));
    end
end