function [Population,FrontNo,MaxFNo] = Reduce_adaptive(Population,sigma)
...
%     ReferPoint = max(Population(LastFront).objs,[],1)*(1+sigma);    % reference point used in hypervolume calculation
    ReferPoint = max(Population(LastFront).objs,[],1)+sigma; 
